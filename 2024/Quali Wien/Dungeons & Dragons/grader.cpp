...
int takeDoor(int k) {
	if (k <= 0 || k > adjl[p].size()) raise("Diese Tuer gibt es nicht");
	steps++;
	if (steps > MAX_STEP) raise("Zu viele Schritte");
	auto e = adjl[p][k - 1];
	log("Von %d zu %d\n", p, e.first);
	p = e.first;
	if (p == n) {
...
void takeNote(string s) {
	if (s.size() > MAX_NOTE) raise("Notiz zu lang");
	note[p] = s;
}

string readNote() {
	// cout << "[MYLOG] (readNote): " << note[p] << endl;
	return note[p];
}
